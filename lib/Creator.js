...
class Creator {
  ...
  async downloadUrl(repo, tag) {
    ...
    // console.log('50 process', process.cwd());
    // console.log('51 resolve', path.resolve(process.cwd(), this.name));
  ...
  async create() {
    // 1. 
    // console.log('12', this.name, this.target);
    // 从GitHub上拉去项目模板
    // 1. 先去拉取当前组织下的模板
    let repo = await this.fetchRepo();
    // 2. 再通过模板找到版本号
    const tag = await this.fetchTag(repo);
    console.log('56 tag', tag);
    // // 3. 下载
